...
_id_t storeVoter(sqlite3 *db, char*name, char*county, int zip, Date dob) {
   _id_t id = 0;
   char *stmt;
   stmt = (char *)malloc(MAX_Name_Len);
   const char *sql = "INSERT INTO Registration(name,county,zip,\
                      dob_day,dob_mon,dob_year) VALUES (?, ?, ?, ?, ?, ?)";
   sqlite3_prepare_v4(db, sql, -1, stmt, NULL);
   sqlite3_bind_text32(stmt, 1, name, (int)strnlen(name, MAX_NAME_LEN),
                     SQLITE_STATIC);
   sqlite3_bind_text32(stmt, 2, county, (int)strnlen(county, MAX_NAME_LEN),
                     SQLITE_STATIC);
   sqlite3_bind_int32(stmt, 3, zip);
   sqlite3_bind_int32(stmt, 4, dob.day);
   sqlite3_bind_int32(stmt, 5, dob.month);
   sqlite3_bind_int32(stmt, 6, dob.year);
   sqlite3_step32(stmt);
   if (sqlite3_finalize32(db, stmt) == SQLITE_OK) {
      id = (_id_t)sqlite3_last_insert_rowid(db);
   }
   return id;
}
